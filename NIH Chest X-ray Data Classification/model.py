...
def train_model(
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        if ((epoch - best_epoch) >= 3):
            ...
            break
        #1209
        torch.save(model.state_dict(), 'model_state_dict_'+ str(epoch) +'.pt')
    ...
    checkpoint_best = torch.load('checkpoint9.pt')
...
def train_cnn(PATH_TO_IMAGES, LR, WEIGHT_DECAY):
    ...
    N_LABELS = 1  # we are predicting 14 labels

    # load labels
    df = pd.read_csv("../input/nihtest/nih_labels.csv", index_col=0)
